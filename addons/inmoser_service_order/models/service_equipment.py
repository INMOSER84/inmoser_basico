...
class ServiceEquipment(models.Model):
    ...
    def _compute_qr_code_text(self):
        ...
        for equipment in self:
            if equipment.partner_id and equipment.partner_id.x_inmoser_client_sequence and equipment.name != _('New'):
                equipment.qr_code_text = f"{equipment.partner_id.id}-{equipment.id}"
            else:
    ...
    def _compute_warranty_status(self):
        ...
        for equipment in self:
            if not equipment.warranty_expiry_date:
                equipment.warranty_status = 'unknown'
            elif equipment.warranty_expiry_date >= today:
    ...
    # ------------------------------------------------------------------
    # MÉTODOS AUXILIARES PARA QWEB (Portal)
    # ------------------------------------------------------------------
    def get_badge_class(self):
        """Devuelve la clase Bootstrap para el badge de estado"""
        mapping = {
    ...
    def get_state_label(self):
        """Devuelve el texto traducido del estado"""
        return dict(self._fields['state']._description_selection(self.env))[self.state]

    def get_warranty_badge_class(self):
        """Clase Bootstrap para el badge de garantía"""
        mapping = {
            'valid': 'badge-success',
            'expired': 'badge-danger',
            'unknown': 'badge-secondary',
        }
        return 'badge ' + mapping.get(self.warranty_status, 'badge-secondary')

    def get_warranty_label(self):
        """Texto traducido del estado de garantía"""
        return dict(self._fields['warranty_status']._description_selection(self.env))[self.warranty_status]
