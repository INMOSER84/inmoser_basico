...
class ServiceReprogramWizard(models.TransientModel):
    ...
    keep_current_technician = fields.Boolean(
        string="Keep Current Technician", default=True
    )
    ...
    notify_technician = fields.Boolean(string="Notify Technician", default=True)
    ...
    def _onchange_new_date(self):
        ...
        if self.new_date:
            ...
            if (
                self.keep_current_technician
                and self.service_order_id.assigned_technician_id
            ):
                current_tech = self.service_order_id.assigned_technician_id
                if not self.service_order_id._is_technician_available(
                    current_tech, self.new_date
                ):
                    ...
                    return {
                        "warning": {
                            ...
                            "message": _(
                                "The current technician is not available on the selected date. Please choose a different technician."
                            ),
                        }
                    }
    ...
    def _validate_reschedule_data(self):
        ...
        final_technician = self._get_final_technician()
        if not self.service_order_id._is_technician_available(
            final_technician, self.new_date
        ):
            raise UserError(
    ...
    def _send_customer_notification(self):
        ...
        if template:
            ...
            template.with_context(ctx).send_mail(
                self.service_order_id.id, force_send=True
            )
