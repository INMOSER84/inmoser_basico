...
class ServiceCompleteWizard(models.TransientModel):
    ...
    service_order_id = fields.Many2one(
        "inmoser.service.order", string="Service Order", required=True, readonly=True
    )
    ...
    photo_before = fields.Binary(
        string="Photo Before Service", help="Photo taken before starting the service"
    )

    photo_after = fields.Binary(
        string="Photo After Service", help="Photo taken after completing the service"
    )

    requires_parts = fields.Boolean(string="Requires Parts/Refactions", default=False)

    refaction_line_ids = fields.One2many(
        "inmoser.service.complete.refaction.line", "wizard_id", string="Parts Used"
    )

    customer_signature = fields.Binary(
        string="Customer Signature", help="Digital signature of customer approval"
    )

    technician_notes = fields.Text(
        string="Technician Notes", help="Additional notes from the technician"
    )

    completion_time = fields.Datetime(
        string="Completion Time", default=fields.Datetime.now, required=True
    )
    ...
    def action_complete_service(self):
        ...
        return {
            ...
            "params": {
                ...
                "message": _("Service order %s has been completed successfully.")
    ...
    def _send_completion_notification(self):
        ...
        if template:
            template.send_mail(self.service_order_id.id, force_send=True)

    def _create_follow_up_activities(self):
        """Crear actividades de seguimiento"""
        # Actividad para call center: seguimiento de satisfacción
        call_center_users = self.env.ref(
            "inmoser_service_order.group_inmoser_call_center"
        ).users
        ...
        if call_center_users:
            self.service_order_id.activity_schedule(
                ...
                summary=_("Customer Satisfaction Follow-up: %s")
                % self.service_order_id.name,
                note=_(
                    "Follow up with customer about service satisfaction. Rating: %s/5"
                )
...
class ServiceCompleteRefactionLine(models.TransientModel):
    ...
    description = fields.Text(
        string="Description", help="Additional description of the part used"
    )
