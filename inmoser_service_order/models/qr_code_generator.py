...
class ServiceOrderQR(models.Model):
    ...
    qr_code_order = fields.Binary(
        string="Order QR Code", help="QR code for accessing this service order"
    )

    qr_code_order_text = fields.Char(
        string="Order QR Code URL", help="URL encoded in the QR code"
    )
    ...
    def action_generate_order_qr_code(self):
        ...
        for record in self:
            try:
                ...
                # Mensaje de éxito
                record.message_post(
                    body=_("Order QR Code generated successfully. URL: %s")
                    % portal_url,
                    message_type="notification",
                )
