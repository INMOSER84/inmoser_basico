...
class HrEmployeeServiceIntegration(models.Model):
    ...
    timesheet_ids = fields.One2many(
        "inmoser.service.timesheet", "employee_id", string="Service Timesheets"
    )

    total_service_hours = fields.Float(
        string="Total Service Hours", compute="_compute_service_statistics"
    )

    monthly_service_hours = fields.Float(
        string="Monthly Service Hours", compute="_compute_service_statistics"
    )
    ...
    virtual_inventory_ids = fields.One2many(
        "inmoser.technician.inventory", "technician_id", string="Virtual Inventory"
    )

    # Configuración de horarios
    service_schedule_ids = fields.One2many(
        "inmoser.technician.schedule", "technician_id", string="Service Schedule"
    )
        ...
        for employee in self:
            ...
            if completed_orders:
                on_time_orders = completed_orders.filtered(
                    ...
                    and o.end_date
                    # 2 horas de tolerancia
                    <= o.scheduled_date + timedelta(hours=2)
                )
                employee.service_efficiency = (
                    len(on_time_orders) / len(completed_orders)
                ) * 100

                # Rating promedio
                rated_orders = completed_orders.filtered(
                    lambda o: o.customer_satisfaction
                )
                if rated_orders:
                    total_rating = sum(
                        int(o.customer_satisfaction) for o in rated_orders
                    )
...
class ServiceTimesheet(models.Model):
    ...
    service_order_id = fields.Many2one(
        "inmoser.service.order", string="Service Order", required=True
    )
...
class TechnicianInventory(models.Model):
    ...
    allocated_quantity = fields.Float(
        string="Allocated Quantity", help="Quantity allocated to this technician"
    )

    available_quantity = fields.Float(
        string="Available Quantity", help="Quantity currently available"
    )
    ...
    last_replenishment = fields.Date(string="Last Replenishment")

    min_quantity = fields.Float(
        string="Minimum Quantity", help="Minimum quantity to maintain"
    )
    ...
    def _compute_used_quantity(self):
        ...
        for inventory in self:
            inventory.used_quantity = (
                inventory.allocated_quantity - inventory.available_quantity
            )
...
class ServiceOrderHRIntegration(models.Model):
    ...
    timesheet_ids = fields.One2many(
        "inmoser.service.timesheet", "service_order_id", string="Timesheets"
    )

    total_timesheet_hours = fields.Float(
        string="Total Hours", compute="_compute_timesheet_hours"
    )
    ...
    def action_start_timesheet(self):
        ...
        if active_timesheet:
            raise UserError(
                _("There is already an active timesheet for this service order.")
            )
    ...
    def action_stop_timesheet(self):
        ...
        return {
            ...
            "params": {
                ...
                "message": _("Timesheet stopped. Total hours: %.2f")
                % active_timesheet.hours,
                "type": "success",
            },
        }
