...
class HrEmployeeExtension(models.Model):
    ...
    x_inmoser_technician_level = fields.Selection(
        ...
        help="Nivel de experiencia del técnico",
    )

    x_inmoser_specialties = fields.Text(
        string="Specialties", help="Especialidades y certificaciones del técnico"
    )
    ...
    def _check_max_daily_orders(self):
        ...
        for employee in self:
            if (
                employee.x_inmoser_is_technician
                and employee.x_inmoser_max_daily_orders <= 0
            ):
                raise ValidationError(
                    _("El número máximo de órdenes diarias debe ser mayor que cero.")
                )
    ...
    def action_view_technician_calendar(self):
        ...
        self.ensure_one()
        action = self.env.ref(
            "inmoser_service_order.action_service_order_calendar"
        ).read()[0]
        action["domain"] = [("assigned_technician_id", "=", self.id)]
        action["context"] = {
            "default_assigned_technician_id": self.id,
    ...
    def get_available_time_slots(self, date):
        ...
        existing_orders = self.x_inmoser_assigned_orders.filtered(
            lambda o: o.scheduled_date
            and o.scheduled_date.date() == date
            and o.state not in ["cancelled", "done"]
        )
    ...
    def check_daily_capacity(self, date):
        ...
        orders_count = len(
            self.x_inmoser_assigned_orders.filtered(
                lambda o: o.scheduled_date
                and o.scheduled_date.date() == date
                and o.state not in ["cancelled", "done"]
            )
    ...
    def get_available_technicians(self, date, service_type=None):
        ...
        for technician in technicians:
            if technician.check_daily_capacity(
                date
            ) and technician.get_available_time_slots(date):
    ...
    def name_get(self):
        ...
        for employee in self:
            ...
            if employee.x_inmoser_is_technician:
                level = dict(
                    employee._fields["x_inmoser_technician_level"].selection
                ).get(employee.x_inmoser_technician_level, "")
                if level:
                    name = f"{name} ({level})"
