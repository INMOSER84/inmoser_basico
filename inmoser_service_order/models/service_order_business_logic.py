...
class ServiceOrderBusinessLogic(models.Model):
        ...
        for record in self:
            ...
            if not record.assigned_technician_id:
                ...
                if available_technician:
                    ...
                    raise UserError(
                        _(
                            "No available technicians found for the scheduled date and time."
                        )
                    )
            ...
            if not record._validate_technician_availability():
                raise UserError(
                    _("The selected technician is not available at the scheduled time.")
                )
            ...
            record.activity_schedule(
                ...
                note=_(
                    "You have been assigned to service order %s for customer %s. Equipment: %s"
                )
    ...
    def action_start_service(self):
        ...
        for record in self:
            ...
            if record.assigned_technician_id.user_id != self.env.user:
                raise UserError(
                    _("Only the assigned technician can start this service.")
                )
    ...
    def action_request_approval(self):
        ...
        for record in self:
            ...
            if (
                not record.refaction_line_ids
                and record.service_type_id.requires_approval
            ):
                raise UserError(
                    _(
                        "At least one refaction line is required for services that require approval."
                    )
                )
    ...
    def action_complete_service(self):
        ...
        for record in self:
            ...
            if record.assigned_technician_id.user_id != self.env.user:
                raise UserError(
                    _("Only the assigned technician can complete this service.")
                )
            ...
            if record.service_type_id.allow_photos and not record.photo_after:
                raise UserError(
                    _("After service photo is required for this service type.")
                )
    ...
    def action_reschedule(self):
        ...
        for record in self:
            if record.state in ["done", "cancelled"]:
                raise UserError(
                    _("Completed or cancelled orders cannot be rescheduled.")
                )
    ...
    def _find_available_technician(self):
        ...
        technicians = self.env["hr.employee"].search(
            [("x_inmoser_is_technician", "=", True), ("active", "=", True)]
        )
        ...
        daily_orders = self.search_count(
            [
                ...
                (
                    "scheduled_date",
                    ">=",
                    scheduled_date.replace(hour=0, minute=0, second=0),
                ),
                (
                    "scheduled_date",
                    "<=",
                    scheduled_date.replace(hour=23, minute=59, second=59),
                ),
                ("state", "not in", ["done", "cancelled"]),
            ]
        )
    ...
    def _validate_technician_availability(self):
        ...
        return self._is_technician_available(
            self.assigned_technician_id, self.scheduled_date
        )
    ...
    def _auto_reschedule_for_parts(self):
        ...
        if available_technician:
            ...
            self.message_post(
                body=_(
                    "Service automatically rescheduled to %s due to parts availability."
                )
                % new_date,
    ...
    def _send_assignment_notification(self):
        ...
        template = self.env.ref(
            "inmoser_service_order.email_template_service_assigned",
    ...
    def _send_service_started_notification(self):
        ...
        self.message_post(
            body=_("Service started by technician %s")
    ...
    def _send_approval_request_notification(self):
        ...
        template = self.env.ref(
            "inmoser_service_order.email_template_approval_request",
            raise_if_not_found=False,
        )
        if template:
            template.send_mail(self.id, force_send=True)
    ...
    def _send_acceptance_notification(self):
        ...
        self.message_post(
            body=_("Service approved by customer"), message_type="notification"
        )
    ...
    def _send_cancellation_notification(self):
        ...
        self.message_post(
            body=_("Service order cancelled"), message_type="notification"
        )
            ...
            if record.assigned_technician_id and record.scheduled_date:
                if not record._validate_technician_availability():
                    raise ValidationError(
                        ...
                        % (record.assigned_technician_id.name, record.scheduled_date)
                    )
    ...
    def _check_refaction_lines(self):
        ...
        for record in self:
            if (
                record.service_type_id.requires_approval
                and record.state == "pending_approval"
            ):
    ...
    def _cron_check_overdue_orders(self):
        ...
        for order in overdue_orders:
            ...
            order.message_post(
                body=_("Service order is now overdue"), message_type="notification"
            )
