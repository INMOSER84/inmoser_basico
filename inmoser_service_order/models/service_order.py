...
class ServiceOrder(models.Model):
    ...
    equipment_id = fields.Many2one(
        "inmoser.service.equipment", string="Equipment", required=True, tracking=True
    )

    service_type_id = fields.Many2one(
        "inmoser.service.type", string="Service Type", required=True, tracking=True
    )
    ...
    diagnosis = fields.Text(
        string="Diagnosis", help="Diagnóstico técnico del problema", tracking=True
    )

    work_performed = fields.Text(
        string="Work Performed", help="Descripción del trabajo realizado", tracking=True
    )
    ...
    customer_signature = fields.Binary(
        string="Customer Signature", help="Firma digital del cliente"
    )

    rejection_reason = fields.Text(
        string="Rejection Reason", help="Motivo del rechazo por parte del cliente"
    )

    # Evidencias fotográficas
    photo_before = fields.Binary(
        string="Photo Before", help="Evidencia fotográfica antes del servicio"
    )

    photo_after = fields.Binary(
        string="Photo After", help="Evidencia fotográfica después del servicio"
    )
    ...
    notes = fields.Text(string="Internal Notes", help="Notas internas del servicio")
    ...
    partner_name = fields.Char(
        related="partner_id.name", string="Customer Name", readonly=True, store=True
    )

    equipment_name = fields.Char(
        related="equipment_id.name", string="Equipment Name", readonly=True, store=True
    )
    ...
    def _compute_total_amount(self):
        ...
        for order in self:
            ...
            base_price = (
                order.service_type_id.base_price if order.service_type_id else 0.0
            )
    ...
    def _compute_can_start(self):
        ...
        for order in self:
            order.can_start = (
                ...
                and order.assigned_technician_id
                and order.scheduled_date
            )
    ...
    def create(self, vals):
        ...
        if vals.get("name", _("New")) == _("New"):
            vals["name"] = self.env["ir.sequence"].next_by_code(
                "inmoser.service.order.sequence"
            ) or _("New")
        ...
        order.activity_schedule(
            ...
            note=_("Service order %s has been created and needs to be processed.")
            % order.name,
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            return {
                "domain": {"equipment_id": [("partner_id", "=", self.partner_id.id)]}
            }
        else:
    ...
    def _onchange_service_type_id(self):
        ...
        if self.service_type_id:
            ...
            if self.scheduled_date and self.service_type_id.estimated_duration:
                estimated_end = self.scheduled_date + timedelta(
                    hours=self.service_type_id.estimated_duration
                )
    ...
    def action_assign_technician(self):
        ...
        if not self.assigned_technician_id.check_daily_capacity(
            self.scheduled_date.date()
        ):
            raise UserError(
                _(
                    "The selected technician has reached the maximum capacity for this date."
                )
            )
    ...
    def action_view_refactions(self):
        ...
        action = self.env.ref(
            "inmoser_service_order.action_service_order_refaction_line"
        ).read()[0]
    ...
    def _create_calendar_event(self):
        ...
        duration = (
            self.service_type_id.estimated_duration if self.service_type_id else 2.0
        )
...
Reported Fault: {self.reported_fault}
            ...
            "user_id": (
                self.assigned_technician_id.user_id.id
                if self.assigned_technician_id.user_id
                else False
            ),
        }
    ...
    def _send_assignment_notifications(self):
        ...
        if self.partner_id.user_ids:
            self.message_post(
                ...
                % (self.name, self.scheduled_date.strftime("%Y-%m-%d %H:%M")),
    ...
    def get_technician_workload(self, technician_id, date_from, date_to):
        ...
        return {
            ...
            "total_estimated_hours": sum(
                order.service_type_id.estimated_duration or 2.0 for order in orders
            ),
        }
