...
class ServiceOrderAccountingIntegration(models.Model):
    ...
    def action_create_invoice(self):
        ...
        self.message_post(
            body=_("Invoice %s created for service order %s")
    ...
    def _prepare_invoice_vals(self):
        ...
        return {
            ...
            % (self.name, self.equipment_id.name, self.service_type_id.name),
    ...
    def _create_invoice_lines(self, invoice):
        ...
        service_line_vals = {
            "move_id": invoice.id,
            "name": _("Service: %s - %s")
            % (self.service_type_id.name, self.equipment_id.name),
            "quantity": 1,
        ...
        for refaction_line in self.refaction_line_ids:
            refaction_line_vals = {
                ...
                "account_id": refaction_line.product_id.property_account_income_id.id
                or refaction_line.product_id.categ_id.property_account_income_categ_id.id,
            }
    ...
    def _get_service_account(self):
        ...
        if not service_account:
            raise UserError(
                _("No income account found. Please configure accounting properly.")
            )
    ...
    def action_create_journal_entry(self):
        ...
        if self.state != "done":
            raise UserError(
                _("Only completed service orders can generate journal entries.")
            )
    ...
    def _get_service_journal(self):
        ...
        if not service_journal:
            raise UserError(
                _("No sales journal found. Please configure accounting properly.")
            )
...
class AccountMoveServiceOrder(models.Model):
    ...
    service_order_count = fields.Integer(
        string="Service Orders Count", compute="_compute_service_order_count"
    )
