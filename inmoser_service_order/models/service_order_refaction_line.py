...
class ServiceOrderRefactionLine(models.Model):
    ...
    quantity = fields.Float(
        string="Quantity", required=True, default=1.0, digits="Product Unit of Measure"
    )
    ...
    description = fields.Text(
        string="Description", help="Descripción adicional de la refacción o trabajo"
    )

    sequence = fields.Integer(
        string="Sequence", default=10, help="Orden de visualización en la lista"
    )

    # Estado y control
    state = fields.Selection(
        related="order_id.state", string="Order State", readonly=True, store=True
    )
    ...
    partner_id = fields.Many2one(
        related="order_id.partner_id", string="Customer", readonly=True, store=True
    )
    ...
    def _compute_available_qty(self):
        ...
        for line in self:
            if (
                line.product_id
                and line.technician_id
                and line.technician_id.x_inmoser_virtual_warehouse_id
            ):
                ...
                stock_quant = self.env["stock.quant"].search(
                    [
                        ...
                        (
                            "location_id",
                            "=",
                            line.technician_id.x_inmoser_virtual_warehouse_id.id,
                        ),
                    ],
                    limit=1,
                )
    ...
    def cancel_reservation(self):
        ...
        if self.stock_move_id and self.stock_move_id.state in [
            "draft",
            "waiting",
            "confirmed",
        ]:
    ...
    def get_most_used_products(self, service_type_id=None, limit=10):
        ...
        """,
            (
                domain
                and f"order_id IN (SELECT id FROM inmoser_service_order WHERE {' AND '.join([f'{d[0]} {d[1]} {d[2]}' for d in domain])})"
                or "1=1",
                limit,
            ),
        )
        ...
        for row in self.env.cr.fetchall():
            ...
            results.append(
                {"product": product, "total_quantity": row[1], "usage_count": row[2]}
            )
