...
class ServiceEquipment(models.Model):
    ...
    brand = fields.Char(
        string="Brand", help="Marca del equipo", required=True, tracking=True
    )

    model = fields.Char(
        string="Model", help="Modelo del equipo (se llena en sitio)", tracking=True
    )
    ...
    location = fields.Char(
        string="Location", help="Ubicación del equipo (técnico)", tracking=True
    )
    ...
    qr_code = fields.Binary(
        string="QR Code", help="Código QR del equipo", readonly=True
    )
    ...
    purchase_date = fields.Date(
        string="Purchase Date", help="Fecha de compra del equipo"
    )

    warranty_expiry = fields.Date(
        string="Warranty Expiry", help="Fecha de vencimiento de la garantía"
    )

    notes = fields.Text(string="Notes", help="Notas adicionales sobre el equipo")
    ...
    def _compute_qr_code_text(self):
        ...
        for equipment in self:
            if (
                ...
                and equipment.partner_id.x_inmoser_client_sequence
                and equipment.name != _("New")
            ):
                equipment.qr_code_text = (
                    f"{equipment.partner_id.x_inmoser_client_sequence}-{equipment.name}"
                )
            else:
    ...
    def _compute_last_service_date(self):
        ...
        for equipment in self:
            ...
            if completed_orders:
                equipment.last_service_date = max(
                    completed_orders.mapped("scheduled_date")
                )
            else:
