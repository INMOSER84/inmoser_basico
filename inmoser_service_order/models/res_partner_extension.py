...
class ResPartnerExtension(models.Model):
    ...
    x_inmoser_phone_mobile_2 = fields.Char(
        string="Mobile Phone 2", help="Teléfono celular adicional del cliente"
    )
    ...
    def _compute_service_order_count(self):
        ...
        for partner in self:
            partner.x_inmoser_service_order_count = len(
                partner.x_inmoser_service_order_ids
            )

    @api.model
    def create(self, vals):
        """Override create para generar secuencia automática si es cliente de servicio"""
        if vals.get("x_inmoser_is_service_client", False) and not vals.get(
            "x_inmoser_client_sequence"
        ):
            vals["x_inmoser_client_sequence"] = self.env["ir.sequence"].next_by_code(
                "inmoser.client.sequence"
            )
        ...
        if vals.get("x_inmoser_is_service_client", False):
            for partner in self:
                if not partner.x_inmoser_client_sequence:
                    vals["x_inmoser_client_sequence"] = self.env[
                        "ir.sequence"
                    ].next_by_code("inmoser.client.sequence")
    ...
    def _check_phone_mobile_2_format(self):
        ...
        for partner in self:
            if partner.x_inmoser_phone_mobile_2:
                # Validación básica de formato de teléfono (números, espacios,
                # guiones, paréntesis, +)
                phone_pattern = r"^[\+]?[0-9\s\-\(\)]+$"
                if not re.match(phone_pattern, partner.x_inmoser_phone_mobile_2):
                    raise ValidationError(
                        _("El formato del teléfono móvil adicional no es válido.")
                    )

    def action_view_equipment(self):
        """Acción para ver los equipos del cliente"""
        self.ensure_one()
        action = self.env.ref("inmoser_service_order.action_service_equipment").read()[
            0
        ]
    ...
    def action_create_service_order(self):
        ...
        action = self.env.ref("inmoser_service_order.action_service_order").read()[0]
        action["views"] = [
            (self.env.ref("inmoser_service_order.view_service_order_form").id, "form")
        ]
        action["context"] = {
            "default_partner_id": self.id,
        }
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        ...
        if name:
            ...
            sequence_partners = self.search(
                [("x_inmoser_client_sequence", operator, name)] + args, limit=limit
            )
    ...
    def name_get(self):
        ...
        for partner in self:
            ...
            if (
                partner.x_inmoser_is_service_client
                and partner.x_inmoser_client_sequence
            ):
