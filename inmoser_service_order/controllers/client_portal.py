...
class InmoserPortal(http.Controller):
    ...
    @http.route(
        ["/inmoser/equipment/<int:equipment_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def equipment_portal(self, equipment_id, **kw):
        ...
        try:
            # Buscar equipo
            equipment = (
                request.env["inmoser.service.equipment"].sudo().browse(equipment_id)
            )
            ...
            service_orders = (
                request.env["inmoser.service.order"]
                .sudo()
                .search(
            ...
            current_order = service_orders.filtered(
                lambda o: o.state not in ["done", "cancelled"]
            )[:1]
            ...
            return request.render(
                "inmoser_service_order.equipment_portal_template", values
            )

        except Exception as e:
            _logger.error(f"Error in equipment portal: {str(e)}")
            return request.render(
                "inmoser_service_order.portal_error", {"error": str(e)}
            )

    @http.route(
        ["/inmoser/service-order/<int:order_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def service_order_portal(self, order_id, **kw):
        ...
        try:
            ...
            return request.render(
                "inmoser_service_order.service_order_portal_template", values
            )
    ...
    def request_service(self, **kw):
        ...
        service_types = (
            request.env["inmoser.service.type"].sudo().search([("active", "=", True)])
        )
    ...
    def _process_service_request(self, **kw):
        ...
        try:
            ...
            return request.render(
                "inmoser_service_order.request_service_template",
                {
                    "error": str(e),
                    "form_data": kw,
                    "service_types": request.env["inmoser.service.type"]
                    .sudo()
                    .search([("active", "=", True)]),
                },
            )
    ...
    def _find_or_create_equipment(self, partner, data):
        ...
        equipment = (
            request.env["inmoser.service.equipment"]
            .sudo()
            .search(
                [
                    ("partner_id", "=", partner.id),
        ...
        if not equipment:
            ...
            equipment = (
                request.env["inmoser.service.equipment"].sudo().create(equipment_vals)
            )

        return equipment

    @http.route(
        ["/inmoser/api/order-status/<int:order_id>"], type="json", auth="public"
    )
    def get_order_status(self, order_id, **kw):
        ...
        try:
            ...
            return {
                ...
                "assigned_technician": (
                    order.assigned_technician_id.name
    ...
    @http.route(
        ["/inmoser/api/technician-location/<int:order_id>"], type="json", auth="public"
    )
...
class InmoserPortalExtended(CustomerPortal):
    ...
    @http.route(
        ["/my/service-orders/<int:order_id>"], type="http", auth="user", website=True
    )
    def portal_service_order_detail(self, order_id, **kw):
        ...
        return request.render(
            "inmoser_service_order.portal_service_order_detail", values
        )
